...
!DO Y = 1, TRA
DO Z = 1, ROWS
!  psd2(X,1,Z) = 10 * log10(psd(X,1,Z)/MAXVAL(psd(X,1,:))) !Relative values
      
   psd2(X,1,Z) = 10 * log10(psd(X,1,Z)) !Absolute values
...
DO Z = 1,ROWS
  ...
  WRITE(40,*) B_SCAN_IMAGE4(X,Y,Z), f_real(X,Y,Z), f_imag(X,Y,Z),&
