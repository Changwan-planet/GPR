PROGRAM CSCAN_GPR_EW

USE MD_BASIC
USE MD_CROSS_CORRELATION
USE MD_INTERPRE

IMPLICIT NONE

!USE GSSI_PROGRAM
INCLUDE "IN_OUT_PATH_CSCAN.h"
INCLUDE "VARIABLES_CSCAN_EW.h"
INCLUDE "READ_FILES_CSCAN.h"
INCLUDE "OPEN_WRITE_CSCAN.h"

DO X = 1, DIS
   DO Y = 1, TRA
!CALCULATE THE BACKGROUND
   CALL background(B_SCAN_IMAGE3(X,Y,:),ROWS,600,900,BGR)  !MALA-160MHz
!   CALL background(B_SCAN_IMAGE3(X,Y,:),ROWS,400,500,BGR) !MALA-450MHz
!   CALL background(B_SCAN_IMAGE3(X,Y,:),ROWS,400,500,BGR) !MALA
 
!REMOVE THE BACKGROUND
     B_SCAN_IMAGE4(X,Y,:) = B_SCAN_IMAGE3(X,Y,:) - BGR
   END DO 
END DO
PRINT*, "BACKGROUND END"

!TO REMOVE THE VALUES IN THE DISTINCT VALUES 
!IN THE FIRST AND SECOND SAMPLES OF A_SCOPE.
 CALL background(B_SCAN_IMAGE4(X,Y,:),ROWS,450,500,BGR2) !GSSI
 CALL background(B_SCAN_IMAGE4(X,Y,:),ROWS,400,450,BGR3) !GSSI


B_SCAN_IMAGE4(:,:,1) = BGR2
B_SCAN_IMAGE4(:,:,2) = BGR3


!THE FIRST DATA FROM AN A-SCOPE OF GPR HAS ZERO VALUE.
!SO THAT I WRITE THE DATA FROM 2 TO REMOVE IT.
!DO Z = 2, ROWS  
!   B_SCAN_IMAGE5(:,:,Z-1) = B_SCAN_IMAGE4(:,:,Z) 
!END DO 


!TOTAL SUM SIGNAL
!DO Z = 1, ROWS2
!   T_SUM_ASCOPE(1,1,Z) = SUM( B_SCAN_IMAGE5(:,:,Z) ) / SIZE( B_SCAN_IMAGE5(:,:,Z) )
!END DO
!PRINT *, "TOTAL_SCAN END"

!CROSS-CORRELATION
!DO Y = 1, TRA
!   DO X = 1, DIS
!      CALL cross_corl(T_SUM_ASCOPE(1,1,:), B_SCAN_IMAGE5(X,Y,:), ROWS2, crrl)
      !PRINT *, "CRRL_N=",CRRL_N
      !PRINT*, "X=",X,"Y=",Y,"MAXLOC=",MAXLOC(crrl)
!     SHIFT(X,Y) = ROWS2 - MAXLOC(crrl,1)  
!    END DO
!     PRINT *,  "TRA=",Y
!END DO
!PRINT *, "CROSS CORRELATION END"

! CALL cross_corl(T_SUM_ASCOPE(1,1,:), B_SCAN_IMAGE5(X,Y,:), ROWS2, crrl)

!ZERO_PADDING FOR MALA-MOGOD, MALA-BOMIN
DO X = 1, DIS
   DO Y = 1,  TRA
      CALL zero_padding(B_SCAN_IMAGE4(X,Y,:), ROWS, ROWS3, B_SCAN_IMAGE6(X,Y,:))
   END DO 
END DO 
PRINT*, "ZERO PADDING END"

!HORIZON PICKING
!S1 = 50
!S2 = 100

!DO Y = 1, TRA
!Y = 1 
!   CALL horizon_pick(B_SCAN_IMAGE6(:,Y,:), DIS, TRA, ROWS3, S1, S2, H_1)
!END DO 
 


INCLUDE "OUTPUT_A_SCAN.h"      !20
!INCLUDE "OUTPUT_B_SCAN.h"      !30
INCLUDE "OUTPUT_C_SCAN.h"       !40
!INCLUDE "OUTPUT_CRRL.h"        !50
!INCLUDE "OUTPUT_INTERPRE.h"    !80

END PROGRAM
