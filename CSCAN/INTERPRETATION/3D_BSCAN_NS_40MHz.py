...
data2_4=data2.reshape(sample,l,point)
#power_h=data2.reshape(sample,l,point)
power_h=np.zeros(shape = (sample,l,point))
...
samples2=list(range(1,sample-1,1))
...
data2_4 = data2_3[:,:,:].T
print(np.shape(data2_3[:,0,:].T))
print(np.shape(data2_4))

#pick horizon based on power
for line in lines:
  for z in samples2: 
    for x in dis: 
        if (data2_4[z-1, line, x] > data2_4[z, line, x] and data2_4[z+1, line, x] > data2_4[z, line, x]): 

           power_h[z,line,x] =  1.0

        else: 
           power_h[z,line,x] = 0.0
...
# d_m = 600    # display_moderator
 ...
 plt.imshow(np.fliplr(data2_4[:,line,:])
