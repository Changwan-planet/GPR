...
DO Y = 1, TRA
DO Z = 1, ROWS
      HILBERT_BSUM_power(X,Y,Z) = SQRT(HILBERT_BSUM(X,Y,Z)**2 + B_SCAN_SUM(X,Y,Z)**2)
END DO 
END DO 
END DO 
...
!DO Z = 1, ROWS
!     SLICE_MEAN2 = SUM(HILBERT_BSUM_power(:,:,Z))/SIZE(HILBERT_BSUM_power(:,:,Z)) 

!   DO X = 1, DIS
!      DO Y = 1, TRA             

!      HILBERT_BSUM_powerdB(X,Y,Z) = 10 * log10( ((HILBERT_BSUM_power(X,Y,Z))**2)/ ((SLICE_MEAN2)**2) )   
...
DO Z = 1,  ROWS
   ...
   HILBERT_BSUM_powerdB(X,Y,Z) = 10 * log( (HILBERT_BSUM_power(X,Y,Z)**2)/(MAXVAL(HILBERT_BSUM_power(X,Y,:))**2) )
...
!            WRITE (62,*) (HILBERT_BSUM_powerdB(X,Y,Z), Z = 1, ROWS)
...
!J=ROWS 
     ...
     DO Z = 1, ROWS
!    DO Y = 1, TRA
        WRITE (62,*) (HILBERT_BSUM_powerdB(X,Y,Z), X = 1, DIS)
!    END DO 
     END DO
