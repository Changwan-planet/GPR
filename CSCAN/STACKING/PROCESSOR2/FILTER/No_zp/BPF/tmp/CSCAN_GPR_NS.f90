...
B_SCAN_IMAGE8 = B_SCAN_IMAGE7_E
!INCLUDE "MANUAL_EDIT_SIGNAL_NS.h" !2023 03 GPR MIHO_ri

!REMOVE AVERAGE
!DO Y = 1, LINE 
!   CALL mean_ascanx(B_SCAN_IMAGE8(:,Y,:), TRA, ROWS,MEAN2(Y,:))
!END DO 

!DO Y = 1, LINE
!DO Z =1 , ROWS
!      B_SCAN_IMAGE8_rmavrg(:,Y,Z) = B_SCAN_IMAGE8(:,Y,Z) - MEAN2(Y,Z)
!END DO 
!END DO 
...
!DO X = 1, TRA
!DO Y = 1, LINE
!   CALL fft(B_SCAN_IMAGE8(X,Y,:), imag, ROWS, f_real(X,Y,:), f_imag(X,Y,:))
...
DO X = 1, TRA
DO Y = 1, LINE
      CALL fft_hilbert(B_SCAN_IMAGE8(X,Y,:),ROWS,HILBERT_SIGNAL) 
