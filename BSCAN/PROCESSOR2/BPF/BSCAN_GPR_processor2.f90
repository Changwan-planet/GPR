...
!CALCUATE THE BACKGROUND
      CALL background(B_SCAN_IMAGE3(X,1,:),ROWS,1,500,BGR) !KOERA
!      CALL background(B_SCAN_IMAGE3(X,1,:),ROWS,501,600,BGR)  !MOGOD
!REMOVE THE BACKGROUND
      B_SCAN_IMAGE4(X,1,:) = B_SCAN_IMAGE3(X,1,:) - BGR
END DO

PRINT *, "B_SCAN_IMAGE3(0,0,0)=", B_SCAN_IMAGE3(0,0,0),"BGR=", BGR
PRINT *, "B_SCAN_IMAGE4(0,0,0)=", B_SCAN_IMAGE4(0,0,0)
...
!   CALL idft(bpf_real(X,1,:), bpf_imag(X,1,:), ROWS, ifft_real(X,1,:), ifft_imag(X,1,:))
!!END DO
